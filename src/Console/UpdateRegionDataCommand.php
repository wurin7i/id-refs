...
class UpdateRegionDataCommand extends Command
{
    ...
    public function handle()
    {
        ...
        if (!$source && $this->confirm('The sql source from github.com/cahyadsn/wilayah will be used. Do you want to continue?', true)) {
    ...
    protected function transformData(): void
    {
        ...
        $result = DB::select("SELECT count(*) AS count FROM wilayah w WHERE w.kode REGEXP '^\\\d{2}\\\.\\\d{2}\\\.\\\d{2}$'");
        $districtChunks = floor($result[0]->count/1000);

        $result = DB::select("SELECT count(*) AS count FROM wilayah w WHERE w.kode REGEXP '^\\\d{2}\\\.\\\d{2}\\\.\\\d{2}\\\.\\\d{4}$'");
